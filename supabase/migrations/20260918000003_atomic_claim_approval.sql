...
BEGIN
  ...
  SELECT id INTO v_deposit_account FROM public.payment_accounts WHERE account_code = '2001-MEMBER-DEPOSIT' FOR UPDATE;
  SELECT id INTO v_bank_account FROM public.payment_accounts WHERE account_code = '1002-BANK-SETTLEMENT' FOR UPDATE;
  IF v_deposit_account IS NULL OR v_bank_account IS NULL THEN
    RAISE EXCEPTION 'Required payout ledger accounts are missing';
  END IF;

  -- The payout transaction is created here instead of calling a second RPC,
  -- so claim approval and ledger posting share one DB transaction.
  ...
  PERFORM public.record_double_entry_ledger(
    ...
    v_bank_account,
    v_payout,
    COALESCE(p_notes, 'Claim payout approved'),
    p_actor_id
  );

  -- Keep the canonical member balance consistent with the existing payout
  -- accounting path. Do not silently create a balance row for a missing member.
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object('claim_id', p_claim_id, 'status', 'approved', 'payout_amount', v_payout, 'transaction_id', v_tx_id),
    p_notes
  );
  ...
  INSERT INTO public.telegram_notification_log (
    ...
    format('Klaim %s disetujui. Payout Rp %s diproses ke %s %s.',
      ...
      COALESCE(v_claim.bank, '-'),
      COALESCE(v_claim.account_number, '-')),
    'claim',
    p_claim_id::text,
    'queued'
  );
